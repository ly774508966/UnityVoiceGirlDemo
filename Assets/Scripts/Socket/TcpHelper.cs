	...
	public class TcpHelper
	{
		...
		private void Accept (IAsyncResult result)
		{
			...
			try {
                ...
                Console.WriteLine("before new ClientInfo ");
                ...
                Log.WriteToLog(string.Format("Client {0} connected", client.RemoteEndPoint));
		...
		private void Recieve (IAsyncResult result)
		{
			...
			if (client == null || !clientPool.ContainsKey (client)) {
                Console.WriteLine("Return Inside Receive ");
				return;
			}
            Console.WriteLine("Inside Receive ");
            try {
				int length = client.EndReceive (result);
                Console.WriteLine("接收到"+length+"个字节");
				...
				if (m.Value != "") { //处理客户端传来的用户名
				    ...
				    CommandDeal.ExecuteCommand(msg);//处理命令
